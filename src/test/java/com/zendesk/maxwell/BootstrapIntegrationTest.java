...
public class BootstrapIntegrationTest extends MaxwellTestWithIsolatedServer {
	...
	public void testStringTypes() throws Exception {
		final String epoch = String.valueOf(new Timestamp(0)); // timezone dependent
	...
	public void testSetType() throws Exception {
		final ArrayList<String> setValue = new ArrayList<>(2);
		setValue.add("a");
		setValue.add("b");
	...
	private void testColumnType(final String sqlType, final String sqlValue, final Object expectedJsonValue) throws Exception {
		testColumnType(sqlType, sqlValue, expectedJsonValue, expectedJsonValue);
	}

	private void testColumnType(final String sqlType, final String sqlValue, final Object expectedNormalJsonValue, final Object expectedBootstrappedJsonValue)
			throws Exception {
		final String[] input = {
		...
		final List<RowMap> rows = getRowsForSQL(input);
	...
	private void testColumnTypeSerialization(final EncryptionMode encryptionMode, final List<RowMap> rows, final Object expectedNormalJsonValue,
			final Object expectedBootstrappedJsonValue) throws Exception {
		...
		for (final RowMap r : rows) {
			...
			final Map<String, Object> decrypted = MaxwellTestJSON.parseEncryptedJSON(output, outputConfig.secretKey);
			...
			if (output.get("table").equals("column_test") && output.get("type").equals("insert")) {
				final Map<String, Object> dataSource = encryptionMode == EncryptionMode.ENCRYPT_DATA ? decrypted : output;
				final Map<String, Object> data = (Map<String, Object>)dataSource.get("data");
				if (!foundNormalRow) {
					foundNormalRow = true;
